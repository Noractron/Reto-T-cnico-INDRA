...
module.exports = {
    async create(req, res) {
        ...
        if (!req.body.swappi_id)
            ...
            //consulta swappi
        await swappi.getPeople(req.body.swappi_id)
            .then(swappis => {
                //valida campos
                if (swappis.nombre == undefined)
                    return res.status(404).send("No se encontro el valor")
                //se calcula el IMC 
                let talla, IMC, composicion;
                talla = Math.pow(swappis.talla / 100, 2);
                IMC = swappis.peso / talla;
